...
new_reviews = ["The movie was a masterpiece", "Worst film I've ever seen",
               "The plot was very engaging", "Terrible acting"]
predicted = model.predict(new_reviews)

# Step 5: Output the predictions
for review, label in zip(new_reviews, predicted):
    print(
        f"Review: '{review}' is classified as: {'Positive' if label == 1 else 'Negative'}")
